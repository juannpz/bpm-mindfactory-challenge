...
describe('TramiteUseCases.crear() - INTERNO_INTERNO', () => {
  ...
  it('should create a tramite with origen INTERNO_INTERNO', async () => {
    const dto = {
      tipoTramiteId: 'tt-001',
      titulo: 'Test Trámite Interno',
      descripcion: 'Descripción de prueba',
      prioridad: Prioridad.MEDIA,
      origen: OrigenTramite.INTERNO_INTERNO,
      areaDestinoId: 'area-001',
    };
    ...
    expect(createdTramite.estado).toBe(EstadoTramite.BORRADOR);
    expect(createdTramite.origen).toBe(OrigenTramite.INTERNO_INTERNO);
    expect(createdTramite.titulo).toBe('Test Trámite Interno');

    expect(response).toHaveProperty('id');
    expect(response).toHaveProperty('numero');
    expect(response.estado).toBe(EstadoTramite.BORRADOR);
    expect(response.origen).toBe(OrigenTramite.INTERNO_INTERNO);
    expect(response.titulo).toBe('Test Trámite Interno');
    expect(response.prioridad).toBe(Prioridad.MEDIA);
