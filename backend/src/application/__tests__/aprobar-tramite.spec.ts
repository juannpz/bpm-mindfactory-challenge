...
describe('WorkflowUseCases.aprobar()', () => {
  ...
  it('should transition tramite to APROBADO and create a movement', async () => {
    const response = await useCases.aprobar(
      't-001',
      { comentario: 'OK' },
      'u-001',
    );
    ...
    expect(updatedTramite.estado).toBe(EstadoTramite.APROBADO);
    ...
    expect(createdMovimiento.accion).toBe(AccionMovimiento.APROBAR);
    expect(createdMovimiento.comentario).toBe('OK');

    expect(response.estado).toBe(EstadoTramite.APROBADO);
    expect(response.id).toBe('t-001');
  });

  it('should work with null comentario', async () => {
    const response = await useCases.aprobar('t-001', {}, 'u-001');

    const updatedTramite: Tramite = mockTramiteRepo.update.mock.calls[0][0];
    expect(updatedTramite.estado).toBe(EstadoTramite.APROBADO);

    const createdMovimiento = mockMovimientoRepo.create.mock.calls[0][0];
    expect(createdMovimiento.comentario).toBeNull();
  ...
  it('should throw if tramite is not found', async () => {
    ...
    await expect(
      useCases.aprobar('t-999', { comentario: 'OK' }, 'u-001'),
    ).rejects.toThrow('Trámite no encontrado');
  });
});
