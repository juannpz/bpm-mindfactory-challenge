...
describe('TramiteUseCases.crear() - INTERNO_EXTERNO', () => {
  ...
  it('should create a tramite with origen INTERNO_EXTERNO', async () => {
    const dto = {
      tipoTramiteId: 'tt-001',
      titulo: 'Trámite hacia externo',
      descripcion: 'Requiere intervención externa',
    ...
    expect(createdTramite.origen).toBe(OrigenTramite.INTERNO_EXTERNO);

    expect(response.origen).toBe(OrigenTramite.INTERNO_EXTERNO);
    expect(response.titulo).toBe('Trámite hacia externo');
    expect(response.prioridad).toBe(Prioridad.URGENTE);
  });
});
